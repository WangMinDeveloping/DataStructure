...
public:
	...
	TValue insert(TKey key, TValue value); // 插入函数,如果插入的值重复，返回以前的改键的旧值
	int deleteNode(TKey key);			   // 删除某键的索引
...
int CRedBlackTree<TKey,TValue>::rightRotate(CRedBlackTreeNode<TKey,TValue> *pCenterNode)
{
	...
	if(pCenterNode==0) return -1;//参数无效
	if(pCenterNode->pLeft==0&&pCenterNode->pRight==0) return 1;//传进来的结点为叶子结点
	if(pCenterNode->pLeft->pLeft==0) return 2;//左子结点为叶子结点
	...
	else pCenterNode->pFather->pLeft= pLeftNode;  // 情况3：如果中心结点是左孩子，更改中心结点父亲的左孩子为中心结点的左孩子
...
int CRedBlackTree<TKey,TValue>::leftRotate(CRedBlackTreeNode<TKey,TValue> *pCenterNode)
{
	...
	if(pCenterNode==0) return -1;//参数无效
	if(pCenterNode->pLeft==0&&pCenterNode->pRight==0) return 1;//传进来的结点为叶子结点
	if(pCenterNode->pRight->pLeft==0) return 2;//右子结点为叶子结点
	...
	else pCenterNode->pFather->pLeft= pRightNode;  // 情况3：如果中心结点是左孩子，更改中心结点父亲的左孩子为中心结点的右孩子
...
{
	CRedBlackTreeNode<TKey,TValue> *pSeekNode=pRootNode;//将根结点的指针赋给pSeekNode
	if(pSeekNode==nullptr) return 0;//根结点为0
	while(pSeekNode->pLeft!=nullptr)
	{
		if(pSeekNode->key==key)//找到了该键
		{
...
/************************************************
  插入函数：
  参数：1、键的地址 ；2、值的地址
  返回值：0 成功；1 new分配空间失败 大于1,说明插入的键已存在，返回以前的该键的值
 **********************************************/
TValue CRedBlackTree<TKey,TValue>::insert(TKey key,TValue value)
{
	...
	if(pExistNode==0)//对应键值不存在
	{
		...
		else 
		{
			...
			pNewNode->pLeft=new CRedBlackTreeNode<TKey,TValue>(pNewNode);//叶子结点
			if(!pNewNode->pLeft) return 1;//空间分配失败
			pNewNode->pRight=new CRedBlackTreeNode<TKey,TValue>(pNewNode);//叶子结点
			if(!pNewNode->pRight) return 1;
...
/************************************************
  ...
  返回值：0 成功；1 删除键不存在
 **********************************************/
int CRedBlackTree<TKey,TValue>::deleteNode(TKey key)
{
	...
	if(pDeleteKeyNode==0) return 1;//没有找到需要删除的结点
	else //找到了需要删除的键值结点
	{
		/*没有指定删除哪一个值*/
