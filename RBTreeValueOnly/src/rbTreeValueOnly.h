...
public:
	...
	*/
	...
	@brief  				删除函数
	@param 		key[in]: 	键
	@return         int:    0 成功  1 删除的键不存在
	*/
	int deleteNode(TKey key);

	/*
	@brief  				查询函数
	@param 		key[in]: 	键
			  value[in]:	值
	@return      TValue:    0-成功 1-键值不存在
	*/
	int find(TKey key, TValue &value);
...
template <class TKey, class TValue>
int CRedBlackTree<TKey, TValue>::find(TKey key, TValue &value)
{
	CRedBlackTreeNode<TKey, TValue> *pSeekNode = pRootNode; // 将根结点的指针赋给pSeekNode
